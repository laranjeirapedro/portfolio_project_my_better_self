...
export const MarketplaceFilterSidebar = () => {
  const { priceFilter, discountFilter, reviewFilter, setDiscountFilter, setPriceFilter, setReviewFilter } = useMarketplaceContext();
  const reviewOptions = [
  ...
  return <Styled.SideBarContainer>
    <Styled.FilterContainer>
      ...
      {reviewOptions.map(option => (
        <Styled.FilterOptionContainer>
          <Styled.RadioButton type="radio" id={option.id} name="review" value={option.value} checked={reviewFilter == option.value} onChange={(e) => {setReviewFilter(Number(e.currentTarget.value))}}/>
          <label htmlFor={option.id}><Styled.FilterOptionContainer>{option.value !== 0 && <Rating rating={option.value}/>}{option.label}</Styled.FilterOptionContainer></label>
        </Styled.FilterOptionContainer>
      ))}
    </Styled.FilterContainer>
    <Styled.FilterContainer>
      ...
      {priceOptions.map(option => (
        <Styled.FilterOptionContainer>
          <Styled.RadioButton type="radio" id={option.id} name="price" value={option.value} checked={priceFilter == option.value} onChange={(e) => {setPriceFilter(Number(e.currentTarget.value))}}/>
    ...
    <Styled.FilterContainer>
      ...
      {discountOptions.map(option => (
        <Styled.FilterOptionContainer>
          <Styled.RadioButton type="radio" id={option.id} name="discount" value={option.value} checked={discountFilter == option.value} onChange={(e) => {setDiscountFilter(Number(e.currentTarget.value))}}/>
          <label htmlFor={option.id}>{option.label}</label>
        </Styled.FilterOptionContainer>
      ))}
    </Styled.FilterContainer>
